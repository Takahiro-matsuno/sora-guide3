var ProjectConverter = function(SignalListener){
    ...
    this.createTrackable = function(enableExtendedTracking) {
        ...
        try {
            if (_this.trackable2DObject && _this.trackable2DObject.destroy) {
                _this.trackable2DObject.destroy();
            }
        } catch (err) {
            Logger.error('CREATE TRACKER - Unexpected error during destroy operation');
            Logger.error(err);
        }
    ...
    /**
     * creates AR.ObjectTrackable by using member arTracker which must be instance of AR.ObjectTracker. Will destroy/reset existing trackable
     */
    this.createObjectTrackable = function() {
        var tracker = _this.arTracker;
        _this.lastTargetName = null;
        $("#disableExtendedTracking").hide();
        ...
        if (_this.trackable3DObject) {
            _this.trackable3DObject.destroy();
        }

        _this.trackable3DObject = new AR.ObjectTrackable(tracker, "*", {
            drawables : {
                cam : []
            },
            onObjectRecognized: onTargetRecognized,
            onObjectLost: onTargetLost
        });
    ...
    this.convert = function(projectUrl) {
        ...
        var initializeProjectJSON = function(projectJSON) { // project.jsの中身
            ...
            try {
                ...
                // 2) create a object/client/cloud- tracker
                try {
                    var isObjectTargetCollection = _this.projectInfo.type && (["MapCollection", "ObjectCollection", "ObjectTargetCollection"].indexOf(_this.projectInfo.type) >= 0);

                    if (isObjectTargetCollection) {
                        if (!isObjectRecoSupported(_this.projectInfo)) {
                            deferred.reject(new Error("VersionConflict"));
                            return;
                        }
                ...
                // 3) set document title to project name, if it exists
                document.title = _this.projectInfo.name ? _this.projectInfo.name : "Wikitude Studio";

                // 4) start the tracker
                _this.arTracker.start();

                _this.signal.dispatch(_this.signal.TYPE.STARTED, _this.projectInfo);

                deferred.resolve();
            } catch (err) {
                Logger.error('Project - Tracker Setup');
                Logger.error(err);
                deferred.reject(err);
            }
        };
    ...
    var onTargetRecognized = function(targetName) {
        ...
        var trackable = _this.trackable2DObject || _this.trackable3DObject;
                            ...
                            if (_this.latestTrackedTargetName !== augmentationsMeta.targetName || _this.latestAugmentationsTimeStamp !== augmentationsMeta.latestAugmentationsTimeStamp) {
                                ...
                                if (_this.latestCamDrawables.length) {
                                    ...
                                    for (var i=_this.latestCamDrawables.length-1; i>=0; i-- ) {
                                        ...
                                        trackable.drawables.removeCamDrawable(i);
                                    }
                                }
                                ...
                                for (var i=0; i<createdAugmentations.length; i++) {
                                    if (createdAugmentations[i]) {
                                        trackable.drawables.addCamDrawable(createdAugmentations[i]);
...
var AugmentationConverter = function() {
    ...
    var convertPrivateAugmentationVideo = function (video, signal) {
            ...
            var handlePlay = function (video, videoDrawable, signal, poster) {
                return function() {
                    videoDrawable.state = "PLAYING";
            ...
            }(video, videoDrawable, signal, poster);
    ...
    /**
     * checks whether passed link is potentially a YouTube link
     * @param uri
     * @returns {*|boolean}
     */
    var isYouTubeLink = function(uri) {
        var youtubeRegex = /^(http(s)??\:\/\/)?(www\.)?((youtube\.com\/watch\?v=)|(youtu.be\/))([a-zA-Z0-9\-_])+/;
        return uri && youtubeRegex.test(uri);
    };

    /**
     * checks whether passed link is potentially a Vimeo link
     * @param uri
     * @returns {*|boolean}
     */
    var isVimeoLink = function(uri) {
        var vimeoRegex = /^(http(s)??\:\/\/)?(www\.)?((vimeo\.com))([a-zA-Z0-9\-_/])+/;
        return uri && vimeoRegex.test(uri);
    };

    /**
     * checks whether passed uri ends with  htm or html (case insensitive)
     * @param uri
     * @returns {*|boolean}
     */
    var isHTMLFile = function(uri) {
        return uri && (uri.toLowerCase().endsWith('.htm') || uri.toLowerCase().endsWith('.html'));
    };

    /**
     * checks whether passed uri is none of the above
     * @param uri
     * @returns {*|boolean}
     */
    var isUriSupportedByMediaPlayer = function(uri) {
        return !( isHTMLFile(uri) || isYouTubeLink(uri) || isVimeoLink(uri) );
    };

    /**
     * launches passed video-url by using Wikitude SDK's video-player or launches browser in case a youtube or vimeo is detected
     * @param uri video url to play
     */
    var startFullscreenVideo = function(uri) {
        if (!uri || !uri.length) {
            Logger.error("Invalid video url passed " + uri);
            return;
        }
        if (!isUriSupportedByMediaPlayer(uri)) {
            AR.context.openInBrowser(uri, true);
        } else {
            AR.context.startVideoPlayer(uri);
        }
    };
        ...
        if (video.properties.autoPlay) {
                ...
                poster.onEnterFieldOfVision =
                    function (video, signal) {
                        return function() {
                            signal.dispatch(signal.TYPE.PLAYED, video);
                            startFullscreenVideo(video.uri);
                        }
                    }(video, signal)
            } else {
                var options = generateDrawable2DOptions(video);
                ...
                var posterResource = new AR.ImageResource(firstFrameUrl);
                poster = new AR.ImageDrawable(posterResource, video.height, options);
    ...
    var convertVideo = function (video, listener, signal) {
        ...
        if (video.properties.videoMode === 'fullscreen' || video.externalUri && !isUriSupportedByMediaPlayer(video.uri)) {
    ...
    var convertModel = function (model, listener, signal) {
        ...
        if (model.properties.isOccluder) {
            drawable = new AR.Occluder(modelUri, options);
        } else {
            drawable = new AR.Model(modelUri, options);
        }
    ...
    var convertImage = function (image, listener, signal) {
        ...
        if (image.properties.clickUrl && image.properties.clickUrl !== "") {
            ...
            var forceNativeBrowser = image.properties.clickUrl.substring(0, 6) === 'mailto'
                || image.properties.forceNativeBrowser === "true";

            var onClickFunction = function (signal, image) {
                return function() {
                    signal.dispatch(signal.TYPE.CLICKED, image);
                    AR.context.openInBrowser(image.properties.clickUrl, forceNativeBrowser);
                }
            }(signal, image);
            // in case the options already contain a click function, append the
            // new click function
